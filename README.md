...
| [`Sourcing/`](Sourcing/) | Ongoing external research pipeline — twice-weekly cloud-scheduled scrape of interview accounts, job descriptions, and resources, staged in an inbox and triaged by hand. Not a testing loop, a feeder. | Whether the whole tracker keeps pace with reality instead of going stale — a book chapter or knowledge-map is only as good as the interview calibration and curriculum data feeding it. | Keeps `Test FDE Role/interview-stories/`, the six category `knowledge-map.md` files, and `AWESOME-FDE-RESOURCES.md` current without a manual research pass every time. The only folder designed to run indefinitely, not toward a completion target. |
